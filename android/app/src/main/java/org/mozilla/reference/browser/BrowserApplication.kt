...
open class BrowserApplication : Application() {
    ...
    override fun onCreate() {
        ...
        WebExtensionSupport.initialize(
            ...
            onExtensionsLoaded = { extensions ->
                ...
                if (hasUnsupportedAddons) {
                    ...
                    // As checks are persistent subscriptions, we have to make sure
        ...
        // Set DuckDuckGo as the default search engine on first run.
        @OptIn(DelicateCoroutinesApi::class)
        GlobalScope.launch(Dispatchers.Main) {
            val prefs = androidx.preference.PreferenceManager.getDefaultSharedPreferences(this@BrowserApplication)
            if (!prefs.getBoolean("ani_duckduckgo_default_set", false)) {
                // Wait for SearchMiddleware to finish populating the store after restore.
                delay(2500)
                val store = components.core.store
                val ddg = store.state.search.searchEngines.firstOrNull { engine ->
                    engine.name.contains("DuckDuckGo", ignoreCase = true)
                }
                if (ddg != null) {
                    components.useCases.searchUseCases.selectSearchEngine(ddg)
                    prefs.edit().putBoolean("ani_duckduckgo_default_set", true).apply()
                    Logger.info("AniBrowser: Default search engine set to DuckDuckGo")
                }
            }
        }
    }
